import { login as loginAction, logout as logoutAction, refreshToken as refreshTokenAction, setVerificationEmail, setForgotPasswordEmail, useAppDispatch, useAppSelector } from "@/redux";
import type { EmailSchema, LoginSchema, OtpSchema, ResetPasswordSchema, SignupSchema } from "@/features/auth";
...
export const useAuth = () => {
    ...
    const { forgotPasswordEmail, verificationEmail, isAuthenticated, user, token } = useAppSelector(store => store.auth)
    ...
    const login = async (payload: LoginSchema, setError: UseFormSetError<any>) => {
        try {
            ...
            if (data.success) {
                loginAction({ user: data.data.user, token: data.data.token })
                nevigate('/')
            }
            else {
                console.log(data);
    ...
    const verifyEmail = async (payload: OtpSchema, setError: UseFormSetError<any>) => {
        try {
            ...
            if (response.data.success) {
                loginAction({ user: response.data.data.user, token: response.data.data.token })
                nevigate('/')
            }
            else {
                console.log(response);
    ...
    const refreashToken = async (payload: { refreshToken: string }) => {
        try {
            ...
            if (data.success) {
                refreshTokenAction({ accessToken: data.data.token.accessToken, refreshToken: data.data.token.refreshToken })
            } else {
                console.log(data);
            }
        } catch (error) {
