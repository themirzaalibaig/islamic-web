...
export const Quran = () => {
    ...
    const formatTime = (seconds: number) => {
        ...
        return formatDuration(duration, { format: duration.hours ? ['hours', 'minutes', 'seconds'] : ['minutes', 'seconds'] })
    }

    return (
        <div className="flex flex-col">
            <header className="border p-4 shadow flex items-center justify-between" style={{ minHeight: `${HEADER_HEIGHT}px` }}>
                <h1 className="text-2xl font-bold text-primary"> <BookOpen className="inline-block mr-2" /> Quran Reader</h1>
                <div className="flex items-center gap-2">
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent className="w-64">
                            ...
                            {recitersList.map((r) => (
                                <DropdownMenuItem key={r.id} onClick={() => handleReciterChange(r.id, selectedSurah, surahs)} className={r.id === reciterId ? 'bg-accent' : ''}>
                                    {r.name}
                                </DropdownMenuItem>
                            ))}
                        </DropdownMenuContent>
            ...
            <main className="flex-1 flex">
                <SurahList
                  ...
                  onSelectSurah={(id) => { fetchVerses(id); loadSurahAudio(id, surahs) }}
                />
                <section className="w-2/3 p-4 app-scrollbar" style={{ height: `calc(100vh - ${HEADER_HEIGHT}px - ${FOOTER_HEIGHT}px  - 108px)` }}>
                    {selectedSurah ? (
            ...
            <footer className="border p-4  flex items-center justify-center" style={{ minHeight: `${FOOTER_HEIGHT}px` }}>
                <div className="flex flex-col gap-2 w-full">
                    <div className="w-full bg-muted rounded-full h-1">
                        <div className="bg-primary h-1 rounded-full transition-all" style={{ width: `${currentAudio.progress}%` }} />
                    </div>
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="font-medium text-sm truncate">
                                ...
                                {currentAudio.type === 'verse' && ` • Verse ${currentAudio.verseKey.split(':')[1] || ''}`}
                            </div>
                        ...
                        <div className="w-40">
                            <Slider value={[Math.round(playbackSpeed * 100)]} min={50} max={200} step={25} onValueChange={(v) => setPlayback(Number((v[0] / 100).toFixed(2)))} />
                        </div>
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button size="sm" variant="ghost" className="text-primary">Reciter</Button>
                            </DropdownMenuTrigger>
                        ...
                        <a href={currentAudio.type === 'surah' ? audioUrl ?? '' : verseAudioRef.current?.src || ''} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground">
                            <Download size={18} />
                        </a>
                        <button onClick={() => refreshCurrentAudio()} className="text-muted-foreground hover:text-foreground" title="Refresh audio">
                            <RefreshCw size={18} />
                        </button>
                        <button
                            onClick={() => {
