  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <Button onClick={onUpdate} variant="outline" disabled={selectedId == null}>
          Update
        </Button>
        <Button onClick={onDelete} variant="destructive" disabled={selectedId == null}>
          Delete
        </Button>
        <Button onClick={api.refetch} variant="outline">
          Refetch
        </Button>
      </div>
      <div className="text-sm">
        Status: {api.status} • Loading: {String(api.isLoading)} • Error: {String(api.isError)}
      </div>
      <ul className="space-y-1">
        {list.map((t) => (
          <li key={t.id} className="border rounded px-2 py-1 flex justify-between">
            <button className="text-left" onClick={() => setSelectedId(Number(t.id))}>
              {t.name} ({t.status})
            </button>
